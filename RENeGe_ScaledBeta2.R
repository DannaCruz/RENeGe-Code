####################################
#### 
#Code for the renege model with prior distribution Scaled Beta2 for variance
##############################################
##############################################
poisson.mtvBETA <-
  function(formula, beta=NULL, phi=NULL, tau2=NULL, rho=NULL, W, Ar,  burnin, n.sample, blocksize.beta=5, blocksize.phi=10, prior.mean.beta=NULL, prior.var.beta=NULL, prior.max.tau2=NULL, t, q, b)
  {
    ##############################################
    #### Format the arguments and check for errors
    ##############################################
    
    #beta=NULL; phi=NULL; tau2=NULL; rho=NULL; burnin=0; n.sample=1000; blocksize.beta=5; blocksize.phi=10; prior.mean.beta=NULL; prior.var.beta=NULL; prior.max.tau2=NULL##########################################
    ...
    ## Global correlation parameter rho de AR
    ...
    K_pos<- SgPrior+(phi)%*%t(phi)/(tau2)
    nu_pos<-nu+n
    Sigma<-rinvwishart(nu_pos, solve(K_pos))
    ...
    #  m     <- rowSums(Ar)
    # Ca        <- diag(1/sqrt(m))%*%Ar%*%diag(1/sqrt(m))
    #lambda   <- eigen(Ca)$values
    #  lamba1<-1/min(lambda)
    # num<--1/2*tr(tau2^(-1)*Covr*solve(Sigma))
    ...
    for(j in 1:n.sample)
    {
      ...
      ##################
      ## Sample from tau2  ESTA ES LA VARIANZA (NO LA PRECISION) DE THETA
      ##################
      
     # tau2.posterior.scale <- 0.5 * sum(phi * (Q %*% phi))
    #  tau2.posterior.shape <- 0.5 * n - 0.5
     # tau2 <- 1/rtrunc(n=1, spec="gamma", a=(1/prior.max.tau2), b=Inf,  shape=tau2.posterior.shape, scale=(1/tau2.posterior.scale))
     
     likelihood = function(param, beta){
        sd = param*solve(Q)
        pred = rep(0, n)
        singlelikelihoods = dmvnorm(phi, mean = pred, sigma = sd,  log = TRUE)
        sumll = sum(singlelikelihoods)
        return(sumll)
      }
      prior = function(param, t=t, q=q, b=b){
        sd = param
        sdprior = log((gamma(t+q)/(b*gamma(t)*gamma(q)))*(param/b)^(t-1)/((param/b)+1)^(t+q))  #SBeta2(0.5, 0.5, 625) 
        return(sdprior)
      }
      
      posterior = function(param){
        return (likelihood(param) + prior(param,  t=t, q=q, b=b))
      }

      proposalfunction = function(param){
        return(abs(rnorm(1,mean = param, sd= c(0.3))))
      }
      
       chain =tau2
   
       proposal = proposalfunction(chain)
          
       probab = exp(posterior(proposal) - posterior(chain))
      ...
      ## Sample from tau2  ESTA ES la varianza DE THETA
      
      #tau2.posterior.scale <- 0.5 * t(phi) %*% Q %*% phi + b_prior_invGamma
      #tau2 <- rgamma(n=1, shape=(0.5*n)+a_prior_invGamma, scale=tau2.posterior.scale)
      #while(tau2 > prior.max.tau2)
      #{
      #  tau2 <- rgamma(n=1, shape=(0.5*n), scale=tau2.posterior.scale)
      #}
      #tau2 <- 1/tau2
      ...
      proposal.CovR<-solve(Mr-proposal.rho*Ar)
      proposal.Covr<-C%*%proposal.CovR%*%t(C)
      posi<-comprPosi(proposal.Covr)
      proposal.SgPrior<-round(proposal.Covr,posi)
      proposal.K_pos<-  proposal.SgPrior+(phi)%*%t(phi)/(tau2)
      nu_pos<-nu+n
      proposal.Sigma<-rinvwishart(nu_pos, solve(proposal.K_pos))
      
      proposal.Q <- solve(proposal.Sigma)
      proposal.det.Q <- 0.5 * as.numeric(determinant(proposal.Q, logarithm=TRUE))[1]
      
      logprob.current <- -tr(SgPrior %*% Q)/2 #actual
      
      logprob.proposal <- -tr( proposal.SgPrior %*% proposal.Q)/2  #propuesta
