####################################
#### 
#Code for the renege model with prior distribution Gamma  0.001,  0.001 for variance
##############################################
poisson.mtv <-
  function(formula, beta=NULL, phi=NULL, tau2=NULL, rho=NULL, W, Ar, C, burnin, n.sample, blocksize.beta=5, blocksize.phi=10, prior.mean.beta=NULL, prior.var.beta=NULL, prior.max.tau2=NULL)
  {
    ##############################################
    ...
    for(j in 1:n.sample)
    {
      ...
      ##################
      ## Sample from tau2  ESTA ES LA PRECISION DE THETA (varianza siempre es INVERSA GAMMA)
      ##################

      
      tau2.posterior.scale <- 0.5 * t(phi) %*% Q %*% phi + 0.001
      tau2 <- rgamma(n=1, shape=(0.5*n)+ 0.001, scale=tau2.posterior.scale)
